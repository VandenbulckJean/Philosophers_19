...
/*   Updated: 2021/09/28 10:50:39 by jvanden-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	initialise_philosopher(int i, t_data *data)
{
	...
	data->assembly.philosophers[i].time_last_meal
		= data->assembly.starting_time;
	data->assembly.philosophers[i].done = 0;
	pthread_mutex_init(&data->assembly.philosophers[i].left_fork, NULL);
	if (data->number_of_philosophers == 1)
		return (0);
	if (i == data->number_of_philosophers - 1)
		data->assembly.philosophers[i].right_fork
			= &data->assembly.philosophers[0].left_fork;
	else
		data->assembly.philosophers[i].right_fork
			= &data->assembly.philosophers[i + 1].left_fork;
	return (1);
}
...
int	initialise(t_data *data)
{
	...
	data->assembly.philosophers = malloc(sizeof(t_philosopher)
